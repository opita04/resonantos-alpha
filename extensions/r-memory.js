...
function resolveApiKeyForProvider(provider) {
  ...
  if (envMap[provider] && process.env[envMap[provider]]) return process.env[envMap[provider]];

  const home = process.env.USERPROFILE || process.env.HOME || "";

  try {
    // 1. Check openclaw.json
    const configPath = path.join(home, ".openclaw", "openclaw.json");
    if (fs.existsSync(configPath)) {
      const conf = JSON.parse(fs.readFileSync(configPath, "utf-8"));

      // Match provider in models.providers
      if (conf.models?.providers) {
        for (const [pName, pConfig] of Object.entries(conf.models.providers)) {
          if (pName === provider || pName.includes(provider) || provider.includes(pName)) {
            // "minimax-oauth" is a placeholder in openclaw.json, skip it to allow falling back to auth-profiles
            if (pConfig.apiKey && pConfig.apiKey !== "minimax-oauth" && pConfig.apiKey !== "openai-oauth") return pConfig.apiKey;
          }
        }
      }

      // Match in env section
      if (conf.env) {
        const upperProv = provider.toUpperCase().replace(/-/g, "_");
        for (const [envName, envVal] of Object.entries(conf.env)) {
          if (envName.includes(upperProv) && envName.includes("API_KEY")) return envVal;
        }
      }
    }

    // 2. Check auth-profiles.json (handles OAuth tokens)
    const agentAuth = path.join(home, ".openclaw", "agents", "main", "agent", "auth-profiles.json");
    if (fs.existsSync(agentAuth)) {
      const data = JSON.parse(fs.readFileSync(agentAuth, "utf-8"));
      if (data.profiles) {
        for (const [key, profile] of Object.entries(data.profiles)) {
          if (key.includes(provider)) {
            const tok = profile?.token || profile?.access || profile?.key;
    ...
    if (fs.existsSync(credDir)) {
      for (const f of fs.readdirSync(credDir)) {
        if (f.includes(provider) && f.endsWith(".json")) {
          ...
          const tok = data.token || data.access || data.key;
          if (tok) { log("INFO", `API key from ${f}`); return tok; }
        }
      }
    }
...
module.exports = function rMemoryExtension(api) {
  ...
  function init() {
    ...
    initialized = true;
    workspaceDir = process.env.OPENCLAW_WORKSPACE || path.join(process.env.USERPROFILE || process.env.HOME || "", ".openclaw", "workspace");
    ensureDir(path.join(workspaceDir, config.storageDir));
